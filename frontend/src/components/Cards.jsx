...
const Cards = ({ card }) => {
  ...
  const [updateCardFormValue, setUpdateCardFormValue] =
  ...
  const handleUpdateCard = (card_id, bucket_id) => {
    if (
      updateCardFormValue.cardName !== "" &&
      updateCardFormValue.link !== "" &&
      updateCardFormValue.linkType !== ""
    ) {
      dispatch(updateCard(updateCardFormValue));
    }
  };

  const handleDeleteCard = (card_id, bucket_id) =>
    dispatch(deleteCard({ card_id, bucket_id }));
